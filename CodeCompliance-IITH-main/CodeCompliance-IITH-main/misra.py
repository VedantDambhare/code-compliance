...
class MisraChecker:
    ...
    def autosar_A_2_7_1(self, rawTokens):
          for token in rawTokens:
            #   print(token.str)
            if token.str.startswith('/*') or token.str.startswith('//'):
            #   a='\\'
            #   print("***")
              ends_with_slash=token.str.endswith('\\')
              if ends_with_slash:
                # print("***")
                self.reportAutosarError(token, 2,7,1,'A')
    ...
    def autosar_M_2_7_1(self, rawTokens):
          for token in rawTokens:
            # print(token.str) 
            if token.str.startswith('/*')  or token.str.startswith('//'):
                # print(token.str)
                tok=token.str
                if '/*' in tok[2:]:
                     self.reportAutosarError(token, 2,7,1,'M')
              
                # print("AUTOSAR RULE A2.7.1 violated")
        #     starts_with_double_slash = token.str.startswith('//')
        #     if token.str.startswith('/*') or starts_with_double_slash:
        #         s = token.str.lstrip('/')
        #         if ((not starts_with_double_slash) and '//' in s) or '/*' in s:
    ...
    #AUTOSAR rules
    def reportAutosarError(self, location, num1, num2,num3,mra):
        ...
        if self.settings.verify:
            ...
            errorId = 'CPP-' + mra + str(num1) + '.' + str(num2) + '.' +str(num3)
            misra_severity = 'Undefined'
            cppcheck_severity = 'style'
            if ruleNum in self.ruleTexts:
                errmsg = self.ruleTexts[ruleNum].text
                ...
                errmsg = 'AUTOSAR violation (use --rule-texts=<file> to get proper output)'
            else:
                errmsg = 'AUTOSAR violation %s with no text in the supplied rule-texts-file' % (ruleNum)
            ...
            if not this_violation in self.existing_violations:
                self.existing_violations.add(this_violation)
                cppcheckdata.reportError(location, cppcheck_severity, errmsg, 'AUTOSAR', errorId, misra_severity)

                if misra_severity not in self.violations:
                    self.violations[misra_severity] = []
                self.violations[misra_severity].append('AUTOSAR-' + errorId)
    ...
    def parseDump(self, dumpfile):
        ...
        for cfgNumber, cfg in enumerate(data.iterconfigurations()):
            ...
            if cfgNumber == 0:
                ...
                # self.executeCheck(301, self.autosar_A_2_7_1, data.rawTokens)    
                self.executeCheck(303, self.autosar_M_2_7_1, data.rawTokens)  
