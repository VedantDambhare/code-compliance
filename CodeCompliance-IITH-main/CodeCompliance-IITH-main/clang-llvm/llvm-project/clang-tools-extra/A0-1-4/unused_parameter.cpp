...
// StatementMatcher callExprDeclaration = callExpr(hasAncestor(functionDecl().bind("caller"))).bind("callee");
DeclarationMatcher FunctionDeclMatcher = functionDecl(hasAnyParameter(varDecl())).bind("method");

class Simple_Pass_Handler : public MatchFinder::MatchCallback {
public:

  virtual void run(const MatchFinder::MatchResult &Result) override {
        if (const FunctionDecl *FD =Result.Nodes.getNodeAs<FunctionDecl>("method")) {
            ...
            for(FunctionDecl::param_const_iterator PI = FD->param_begin(),PE = FD->param_end(); PI != PE; ++PI) {
                ...
                if (param -> isThisDeclarationReferenced() == false) {
                  ...
                  unsigned DiagID = Diagnostics.getCustomDiagID(DiagnosticsEngine::Error, " Violated the rule A0-1-4 - Method Parameter unused: ");
