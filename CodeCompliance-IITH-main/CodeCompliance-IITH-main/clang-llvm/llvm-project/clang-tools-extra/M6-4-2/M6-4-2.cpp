  ...
  virtual void run(const MatchFinder::MatchResult &Result) override {
    ...
    if (const clang::IfStmt *st = Result.Nodes.getNodeAs<clang::IfStmt>("node")){
      if (st -> hasElseStorage() == false) {
        ...
        unsigned DiagID = Diagnostics.getCustomDiagID(DiagnosticsEngine::Error, "AUTOSAR C++ (Rule M6-4-2): Violated All if ... else if constructs shall be terminated with an else clause ");
