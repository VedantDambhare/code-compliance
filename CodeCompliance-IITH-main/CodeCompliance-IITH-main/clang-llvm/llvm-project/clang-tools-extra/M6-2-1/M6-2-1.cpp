...
StatementMatcher StatementMatch1 = ifStmt(hasCondition(hasDescendant(binaryOperator(hasOperatorName("="))))).bind("if-node");

StatementMatcher DeclMatcher = binaryOperator(hasOperatorName("="),hasDescendant(declRefExpr()),hasDescendant(binaryOperator(hasOperatorName("=")))).bind("decl-ref");
...
int main(int argc, const char **argv) {
  ...
  MatchFinder Finder;
  Finder.addMatcher(StatementMatch1, &Handler);
  Finder.addMatcher(DeclMatcher,&Handler);
