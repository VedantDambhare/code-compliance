...
public:
  ...
  virtual void run(const MatchFinder::MatchResult &Result) override {
     if (const FunctionDecl *FD =Result.Nodes.getNodeAs<FunctionDecl>("function")) { 
        ...
        if (((FD ->isMain() == false) && ( FD -> isStatic())) || ( (FD -> isCXXClassMember() == true) && (FD -> getAccess() == clang::AccessSpecifier::AS_private) ) || (FD ->isInAnonymousNamespace() == true)) {
            if (FD -> isReferenced() == false) {

                unsigned DiagID =
                Diagnostics.getCustomDiagID(DiagnosticsEngine::Error,
                  " AUTOSAR C++ (Rule A0-1-3):Violated, Function %0 which is either static, Anonymous or private member is unused");
                Diagnostics.Report(FD ->getBeginLoc(), DiagID).AddString(FD -> getNameAsString());
            }   
        }
